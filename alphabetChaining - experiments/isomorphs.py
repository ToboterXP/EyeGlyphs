...
def detectIsomorphs(messages, minIsomorphLength=5, maxIsomorphLength=50, maxDotRatio = 0.6, overridePos = []):
    ...
    for isomorphLength in range(minIsomorphLength, maxIsomorphLength+1):
        for h in range(len(messages)):
            ...
            while i <= len(messages[h])-isomorphLength:
                ...
                if overridePos and not (h,i) in overridePos:
                    i+=1
                    continue
...
def main():
    ...
    #isos.update(detectIsomorphs(messages, maxIsomorphLength = 50, maxDotRatio = 0.9), overridePos = [(3,18), (4,24),(5, 23)])
    ...
    print(isoSeqs)
    print("Isomorph Amount:",len(isos))
