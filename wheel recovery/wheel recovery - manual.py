...
messages = (
    ...
    Message("mnopqrstuvwxyz0123456789","\\2kK\\[c_JQAHaom'#:^?n:YeH",73),
    Message("+%*/#!$.","VokPVW3^",35),
    
    Message("abcdefghijkl","RY>gk:dVYXRg",53), #m8
    Message("mnopqrstuvwxyz0123456789","bOUBb7i?VFmc+_o&65Sej5%1c",76),
    Message("+%*/#!$.","G1jqG.n ",36),
    
    Message("abcdefghijkl","'B@>?3:(BN'>",52), #m9
    Message("mnopqrstuvwxyz0123456789","9MJp9,6l4S^5H)I*Li(Afi&?5",74),
    Message("+%*/#!$.","V%QPVWT^",35),
    )


class WheelSection:
    def __init__(self,trigram,equivalenceClass,charOffsets=[],trigramOffsets=[],charsByOffset={},trigramsByOffset={}):
    ...
    def addCharOffset(self,charOffset):
        global addEquivalence, getEquivalent

        o = self.charsByOffset.get(charOffset.offset)
        if o:
            if o.getCharacter() != charOffset.getCharacter():
                self.equivalenceClass.addEquivalence(o.getCharacter(), charOffset.getCharacter())
            return
        else:
            charOffset.setEquivalenceClass(self.equivalenceClass)
    ...
    def mergeWheelSection(self, other, offset):
        ...
        for char in other.charOffsets:
            self.addCharOffset(CharOffset(char.getCharacter(), char.offset + offset))
    ...
    def getCharOffsets(self,char):
        return list(filter(lambda c: c.getCharacter() == char, self.charOffsets))

    def getTrigramOffset(self,tri):
        if self.trigram == tri:
            return TrigramOffset(tri, 0)

        for triOff in self.trigramOffsets:
    ...
    def copy(self):
        return WheelSection(self.trigram, list(map(lambda a: a.copy(), self.charOffsets)), list(map(lambda a: a.copy(), self.trigramOffsets)), copy.deepcopy(self.charsByOffset), copy.deepcopy(self.trigramsByOffset))

    def __repr__(self):
        return f"<Wheel Section {self.trigram}>"
...
class EquivalenceClass:
    ...
    def addEquivalence(self,a,b):
        for e in self.equivalences:
            ...
            self.equivalences.append([a,b])

    def getEquivalent(self,a):
        for e in self.equivalences:
            if a in e:
                return e[0]
        return a
    ...
    def copy(self):
        new = CharacterEquivalenceClass()
        new.equivalences = copy.deepcopy(self.equivalences)
        return new

EquivalenceClass.Empty = EquivalenceClass()
   
class CharOffset:
    def __init__(self, character, offset, equivalenceClass=EquivalenceClass.Empty):#offset = how far before that trigram
        global getEquivalent
        self.equivalenceClass = equivalenceClass
        self.character = character
        self.offset = offset % 83

    def setEquivalenceClass(self,eClass):
        self.equivalenceClass = eClass

    def getCharacter(self):
        return self.equivalenceClass.getEquivalent(self.character)

    def __str__(self):
        return f"chr {self.getCharacter()} {self.offset}"

    def copy(self):
        return CharOffset(self.getCharacter(), self.offset)
...
generalEquivalenceClass = EquivalenceClass()            

#step one: extract character to trigram offsets
for m in messages:
    for i in range(len(m.plaintext)):
        offset = (m.offset + i)%83
        trigram = m.trigrams[i]
        character = generalEquivalenceClass.getEquivalent(m.plaintext[i])
        ...
        for s in sections:
            ...
            section = WheelSection(trigram,generalEquivalenceClass,[])
...
#step two: extract trigram offsets from offset repeats in gap patterns
trigramEquivalences = (
    (("O",40),("g",68),("d",70),("&",45),("I",80)),
    (("P",42),("j",70),("1",72),("`",47),("Y",82)),
    (("3",44),("$",72),("-",74),("Q",49),("7",84)),
...
for triEqs in trigramEquivalences:
    baseTri, baseOff = triEqs[0]
    baseSection = None
    
    for sect in sections:
        if sect.getTrigramOffset(baseTri):
            baseSection = sect
            break

    for newTri, newOff in triEqs[1:]:
        newSect = None

        for sect in sections:
            if sect.getTrigramOffset(newTri):
                newSect = sect
                break

        print(baseTri, newTri)
        offset = baseOff + baseSection.getTrigramOffset(baseTri).offset - newOff - newSect.getTrigramOffset(newTri).offset
        baseSection.mergeWheelSection(newSect, offset)
        sections.remove(newSect)
...
trigrams = list(map(lambda a: a.trigram, sections))
    

alphabet = []
for s in sections:
    for c in s.charOffsets:
        if not c.getCharacter() in alphabet:
            alphabet.append(c.getCharacter())

alphabet.sort()


#manual merge decisions! yay...
def mergeSections(tri1, tri2, offset):
    sect1 = tuple(filter(lambda a: a.trigram == tri1, sections))[0]
    sect2 = tuple(filter(lambda a: a.trigram == tri2, sections))[0]
...
##
###based on F 45 C 42
##mergeSections("P","j",45-73)
##mergeSections("P","1",45-75)
##mergeSections("P","`",45-50)
##mergeSections("P","Y",45-2)
##
###based on I 48 E 44
##mergeSections("3","-",48-78)
##mergeSections("3","$",48-76)
##mergeSections("3","Q",48-53)
##mergeSections("3","7",48-5)
##
###based on H 47 A 40
##mergeSections("O","g",47-75)
##mergeSections("O","d",47-77)
##mergeSections("O","&",47-52)
##mergeSections("O","I",47-80)
##
###based on X 54 S 49
##mergeSections("P","^",54-6)
##mergeSections("P","*",54-62)
##mergeSections("P","%",54-14)
##
##
##
for a,b in combinations(alphabet,2):
    ...
    distances = []
    for s in sections:
        for aOff,bOff in combinations(s.charOffsets,2):
            off = (aOff.offset - bOff.offset)%83
            if aOff.getCharacter() == a and bOff.getCharacter() == b:
                distances.append((off,s))
                if not off in distancesD:
                    distancesD.append(off)
    distancesD = list(distancesD)
    distancesD.sort()
    numDistances = list(map(lambda a: a[0],distances))
    #if len(distancesD) < len(numDistances):
    if len(numDistances)>=2:
        print(a,b,distances)
...
print("Equivalent values:",generalEquivalenceClass.equivalences)

alphabet = set()

for m in messages:
    for c in m.plaintext:
        alphabet.add(generalEquivalenceClass.getEquivalent(c))

print("Alphabet:",len(alphabet))

sections.sort(key = lambda a: a.trigram, reverse =False)

print(f"Wheel Sections (character or trigram, value, offset) ({len(sections)}):")
print()
for sect in sections:
