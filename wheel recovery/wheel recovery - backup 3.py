...
class WheelSection:
    ...
    def addCharOffset(self,charOffset):
        ...
        charOffset.charater = getEquivalent(charOffset.character)
        o = self.charsByOffset.get(charOffset.offset)
        if o:
            if getEquivalent(o.character) != charOffset.character:
...
def attemptMergePattern(merges,allSections,mergePattern,explorative_start=0):
    global equivalences, UPDATE_SECTIONS_ON_EQUIVALENCE
    UPDATE_SECTIONS_ON_EQUIVALENCE = False

    merges = merges[:]

    allSections = allSections.copy()

    replaceSects = {}

    for i in range(len(merges)):
        if mergePattern[i]:
            sect1, sect2, offset = merges[i]
            

            while sect1 in replaceSects.keys():
                sect1 = replaceSects.get(sect1,sect1)

            while sect2 in replaceSects.keys():
                sect2 = replaceSects.get(sect2,sect2)

            if not sect1 or not sect2:
                continue
            ...
            try:
                ...
                return i, allSections

            replaceSects[sect1] = mergedSection
            replaceSects[sect2] = None
        ...
        if i>=explorative_start:
            for sect1a, sect2a in permutations(allSections,2):
                continue
                for offset in range(83):
                    break
                    if not sect1a.checkForTrigramCollisions(sect2a, offset):
                        break
                else:
                    return i, allSections
...
def getAffectedBy(merges, errorIndex):
    affectedSections = {merges[errorIndex][0], merges[errorIndex][1]}
    affectedIndices = [errorIndex]
    for i in range(errorIndex-1,-1,-1):
        if merges[i][0] in affectedSections:
            affectedSections.add(merges[i][1])
            affectedIndices.append(i)
        if merges[i][1] in affectedSections:
            affectedSections.add(merges[i][0])
            affectedIndices.append(i)
    return list(set(affectedIndices))
        

def mergeCollisionless(merges,allSections):
    global equivalences, UPDATE_SECTIONS_ON_EQUIVALENCE

    mergePattern = [True]*len(merges)

    triCount = 0
    for sec in allSections:
        triCount += len(sec.trigramOffsets)+1

    print("Trigrams at start:",triCount)

    allSections = set(allSections)

    startingEquivalences = copy.deepcopy(equivalences)

    errorIndex, newAllSections = attemptMergePattern(merges,allSections,mergePattern)
    print(f"Problem at {errorIndex} ({len(newAllSections)} remaining)")

    triCount = 0
    for sec in newAllSections:
        triCount += len(sec.trigramOffsets)+1

    print("Trigrams at middle:",triCount)

    

    while errorIndex != None:
        affectedBy = getAffectedBy(merges, errorIndex)
        print(affectedBy)
        print(f"{len(affectedBy)} indices available")
        for i in range(1,len(affectedBy)+1):
            print(f"Trying {i} changes")
            end = False
            for all_i in combinations(affectedBy, i):
                newMergePattern = mergePattern[:]
                for j in all_i:
                    newMergePattern[j] = not newMergePattern[j]
                    

                equivalences = copy.deepcopy(startingEquivalences)
                newIndex, newAllSections = attemptMergePattern(merges, allSections, newMergePattern, errorIndex)
                if newIndex and newIndex > errorIndex:
                    errorIndex = newIndex
                    mergePattern = newMergePattern[:]
                    print(f"New breakthrough to {errorIndex} ({len(newAllSections)} remaining)")
                    triCount = 0
                    for sec in newAllSections:
                        triCount += len(sec.trigramOffsets)+1
    ...
    print("Trigrams at end:",triCount)
    return list(map(lambda a: a.copy(), newAllSections))
...
while changes:
    ...
    for sect1, sect2 in combinations(sections,2):
        possibleMerges = []
