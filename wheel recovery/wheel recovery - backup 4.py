...
class WheelSection:
    ...
    def addCharOffset(self,charOffset):
        ...
        if o:
            if self.equivalenceClass.getEquivalent(o.getCharacter()) != charOffset.getCharacter():
...
class EquivalenceClass:
    ...
    def copy(self):
        ...
        new.equivalences = self.equivalences.copy()
        return new
...
while changes:
    print(f"Starting iteration ({len(sections)} sections remaining)")
    changes = False
    ...
    for a,b in combinations(alphabet,2):
        ...
        if len(distancesD) < len(numDistances):
            for n in distancesD:
                if numDistances.count(n)>1:
                    multipliedOffsets.append((a,b,n, list(map(lambda a: a[1] ,filter(lambda a: a[0]==n, distances)))))

    for a,b,n, sects in multipliedOffsets:
        if (a,b,n) in excludedOffsets:
            continue
        baseSect = None
        baseOffset = None
        ...
        for s in sects:
            ...
            aOffs = []
            for aOff in s.getCharOffsets(a):
                for bOff in s.getCharOffsets(b):
                    if (aOff.offset-bOff.offset)%83 == n:
                        aOffs.append(aOff.offset)
            ...
            if aOffs:
                ...
                if not baseSect:
                    ...
                    baseSect.mergeWheelSection(s, baseOffset-offset)
                    changes = True
                    sections.remove(s)
