...
while changes:
    changes = False
    collisionless_alphabet = list(alphabet)
    charInSections = {}
    charNotInSections = {}

    for char in alphabet:
        charInSections.setdefault(char,list())
        charNotInSections.setdefault(char,list())


    for sect1, sect2 in permutations(sections,2):
        for char in collisionless_alphabet[:]:
            ...
            for sect1char in sect1chars:
                for sect2char in sect2chars:
                    ...
                    if sect1.checkForTrigramCollisions(sect2, sect1char.offset - sect2char.offset):
                        if not (sect1,sect1char) in charNotInSections[char]:
                            charNotInSections[char].append((sect1,sect1char))
                        if not (sect2,sect2char) in charNotInSections[char]:
                            charNotInSections[char].append((sect2,sect2char))
                        continue

                    noSuccess = False

                    if not (sect1,sect1char) in charInSections[char]:
                        charInSections[char].append((sect1,sect1char))

                    if not (sect2,sect2char) in charInSections[char]:
                        charInSections[char].append((sect2,sect2char))
                        
            if noSuccess:
                collisionless_alphabet.remove(char)
                continue
    ...
    for char in collisionless_alphabet:
        ...
        if char in used:
            continue
        used.append(char)
        sects = list(filter(lambda a: a[0] in sections and a not in charNotInSections[char], charInSections[char]))
        if sects:
            mainSect = sects[0][0]
