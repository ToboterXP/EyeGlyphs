...
for i in range(100):
    for j1 in range(len(messages)-1):
        for j2 in range(j1+1, len(messages)):
            if len(messages[j1]) > i and len(messages[j2]) > i:
                c += 1
                if messages[j1][i] == messages[j2][i]:
                    P += 1
...
assumedIoC = 1.92
assumedAlphabetSize = 26

cs = cpcs/(assumedIoC/assumedAlphabetSize)
print(1/cs)
