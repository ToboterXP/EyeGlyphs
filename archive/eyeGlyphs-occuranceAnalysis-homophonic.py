...
def analyse(name,dataset):
    ...
    print(name, min(dataset), round(lower_quart,2), round(avg,2), median, round(upper_quart,2), max(dataset))
...
for itr in range(n):
    ...
    if RANDOM_MESSAGE:
        #generate homophonic cipher
        letterFreqs = (6, 1, 3, 3, 9, 2, 1, 4, 6, 1, 1, 3, 2, 6, 6, 2, 1, 5, 6, 7, 2, 1, 2, 1, 1, 1)
        assignedTris = []
        tris = list(range(83))

        random.shuffle(tris)
        for l in letterFreqs:
            assignedTris.append([])
            for i in range(l):
                assignedTris[-1].append(tris.pop())

        #generate transmutation cipher
        columnCount = 10
        tmap = list(range(columnCount))
        random.shuffle(tmap)
        ...
        for i in range(len(oldm)):
            ...
            for j in range(len(oldm[i])):
                ...
                for k in range(i):
                    ...
                    ind = random.randint(0,82)
                    for l in range(len(letterFreqs)):
                        ind -= letterFreqs[l]
                        if ind < 0:
                            nl = assignedTris[l].pop()
                            assignedTris[l].insert(0,nl)
                            messages[-1].append(nl)
                            break
                        
            #apply random transmutation
            m = messages[-1]
            nm = []
            for i in range(0, len(m), columnCount):
                start = i
                stop = i+columnCount
                if stop < len(m):
                    s = m[start:stop]
    ...
    s = list(map(lambda a: len(a), s))
    
    for i in range(83):
        p[i].append(s.count(i))
