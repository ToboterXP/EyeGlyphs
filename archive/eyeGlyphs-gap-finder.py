...
def analyse(name,dataset):
    ...
    print(name, round(min(dataset),2), round(lower_quart,2), round(avg,2), round(median,2), round(upper_quart,2), round(max(dataset),2))
    ...
    for i in range(len(oldm)):
        ...
        for j in range(len(oldm[i])):
            ...
            for k in range(i):
                ...
                n = random.randint(0,82)
                messages[-1].append(n)
                    
for j in range(1,51):
    print(j,end=": ")
    for m,n in zip(messages, messageNames):
        for i in range(len(m)-j):
            if m[i] == m[i+j]:
                print("(%s %d %d)" % (n,i+1,i+j+1),end=" ")
    print()
