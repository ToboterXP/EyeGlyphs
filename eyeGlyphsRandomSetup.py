...
def IoC(messages, valRange = (0,82), precalc=False):
...
for itr in range(n):
    ...
    for dist1 in range(0,10):
        for dist2 in range(1,10):
            ...
            for m in messages:
                messageSums.append([])
                for i in range(max(dist1, dist2), len(m)):
                    if m[i] != -1 and m[i-dist1] != -1 and m[i-dist2] != -1:
                        messageSums[-1].append( (m[i-dist1] - m[i-dist2] + m[i]) % 83)
                        


            p[dist1*10 + dist2].append(IoC(messageSums, (0,82), True))
