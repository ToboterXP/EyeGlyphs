...
class AlphabetChain:
    ...
    def getForceSpread(self,newSpread):
        ...
        for offset, spread, chain in self.atomicChains:
            newChains.append((offset, spread * newSpread, chain))
...
##for c1, c2 in itertools.combinations(chains, 2):
##    shared = set()
##    for c in c1.compiledSequence:
##        if c in c2.compiledSequence:
##            shared.add(c)
##
##    if shared:
##        print(c1.compiledSequence,c2.compiledSequence, shared)
...
for bigramSpread in range(-84,83):
    if bigramSpread == 0:
        continue
    messageBigrams = {}

    for m in messages:
        for i in range(max(50-bigramSpread, 50), min(len(m), len(m)-bigramSpread)):
            bigram = (m[i], m[i+bigramSpread])
            messageBigrams.setdefault(bigram, 0)
            messageBigrams[bigram] += 1


    for chain in chains:
        seq = chain.compiledSequence
        n = 0
        for i in range(len(seq)-1):
            bigram = (seq[i], seq[i+1])
            if messageBigrams.get(bigram):
                n += messageBigrams[bigram]

        bigramSpreads.setdefault(chain, [])
        bigramSpreads[chain].append((bigramSpread, n))

spreadChains = []


badSpreads = {(44, 13, 63) : [-4], (62, 64, 21, 13, 19, 4) : [-2, -21, 2], (18, 45, 34, 55) : [-1], (68, 26, 19):[-1, -3], (25, 68, 71):[-3, 5, 8, -1], (4, 60, 25):[3]}
irrelevantChains = ((6, 14, 47), (8, 54, 39))#{6, 14, 47}, {8, 54, 39} can be ignored, can't ever be merged anyways

for chain in chains:
    if tuple(chain.compiledSequence) in irrelevantChains:
        continue
    
    spreads = bigramSpreads[chain]
    spreads = tuple(filter(lambda s: not s[0] in badSpreads.get(chain.compiledSequence, []), spreads))
    maxBis = max(map(lambda s: s[1], spreads))
    bestSpreads = tuple(filter(lambda s: s[1] == maxBis, spreads))

##    if len(bestSpreads) > 5:
##        continue
    
    smallestBestVal = min(map(lambda s: abs(s[0]), bestSpreads))
    smallesBestSpread = tuple(filter(lambda s: abs(s[0]) == smallestBestVal, bestSpreads))[0]

    print(chain.compiledSequence, smallesBestSpread, bestSpreads)

    spreadChains.append(chain.getForceSpread(smallesBestSpread[0]))
    #print(chain.compiledSequence, tuple(filter(lambda s: s[1] == maxBis, spreads)))
...
while True:
    ...
    for c1, c2 in itertools.combinations(chains, 2):
        ...
        combined = c1.mergeOnSingleMatch(c2)
        if combined and not combined.compiledSequence in newSequences:
            newChains.append(combined)
            newSequences.add(combined.compiledSequence)
            processed.add(c1)
