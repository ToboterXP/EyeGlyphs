...
class AtomicChain:
    ...
    def __repr__(self):
        return f"[AtomicChain {self.isomorph} {self.isomorphIndex} {self.sequence}]"

    def getInverted(self):
        return AtomicChain(isomorph = self.isomorph, sequence = self.sequence[::-1], isomorphIndex = self.isomorphIndex)
...
class AlphabetChain:
    ...
    def getFlattened(self):
        return AlphabetChain(atomicChain = AtomicChain(isomorph = None, sequence = self.compiledSequence, isomorphIndex = None))
...
for isomorph, iChains in isomorphs:
    for chain in iChains:
        chains.append(AlphabetChain(atomicChain = AtomicChain(isomorph = isomorph, sequence = chain[:3], isomorphIndex = chain[3])))
